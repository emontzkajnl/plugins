...
class Handling{
	...
	public static function handling_request_with_confirmation($email, $profile, $type=NULL){
		...
		try{
			...
			$json['success'] = __('A notification has been sent to your email. Please confirm your subscription from there.', 'masterstudy');
			echo json_encode($json);
			exit;
		}catch(Mailchimp_List_AlreadySubscribed $e){
			$json['error'] = __('User with this email is already subscribed', 'masterstudy');
			echo json_encode($json);
			exit;
		}catch(Mailchimp_List_MergeFieldRequired $e){
			$json['error'] = __('Please go to http://mailchimp.com/, login, go to your list, then Settings, and put \"First Name\" and \"Last Name\" not required.', 'masterstudy');
			echo json_encode($json);
			exit;
		}catch(Mailchimp_Invalid_ApiKey $e){
			$json['error'] = __('Your API Key is Invalid!', 'masterstudy');
			echo json_encode($json);
			exit;
		}catch(Mailchimp_List_DoesNotExist $e){
			$json['error'] = __('The list that you provided does not exists!', 'masterstudy');
			echo json_encode($json);
			exit;
		}catch(Exception $e){
			#Send error back to developer
