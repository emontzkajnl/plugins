...
export const hasNonPostEntityChanges = createRegistrySelector(select => state => {
  ...
  return some(dirtyEntityRecords, entityRecord => entityRecord.kind !== 'postType' || entityRecord.name !== type || entityRecord.key !== id);
});
/**
...
export const isEditedPostAutosaveable = createRegistrySelector(select => state => {
  ...
  const currentUserId = get(select(coreStore).getCurrentUser(), ['id']); // Disable reason - this line causes the side-effect of fetching the autosave
...
export const isSavingNonPostEntityChanges = createRegistrySelector(select => state => {
  ...
  return some(entitiesBeingSaved, entityRecord => entityRecord.kind !== 'postType' || entityRecord.name !== type || entityRecord.key !== id);
});
/**
...
export const __experimentalGetDefaultTemplateType = createSelector((state, slug) => find(__experimentalGetDefaultTemplateTypes(state), {
  slug
...
export function __experimentalGetTemplateInfo(state, template) {
  ...
  const templateTitle = isString(title) ? title : title === null || title === void 0 ? void 0 : title.rendered;
  const templateDescription = isString(description) ? description : description === null || description === void 0 ? void 0 : description.raw;
