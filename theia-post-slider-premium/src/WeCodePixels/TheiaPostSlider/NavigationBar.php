...
class NavigationBar {
	...
	public static function get_navigation_barButton( $options, $direction ) {
		...
		$numberOfPages = is_array($pages) ? count($pages) : 1;
		...
		if ( ! $url && Options::get( 'button_behaviour', 'tps_nav' ) == 'loop' ) {
			if ( $direction_name == 'next' ) {
				...
				$url = Misc::get_post_page_url( $numberOfPages );
			}
		}
		...
		if ( Options::get( 'button_behaviour' ) === 'post' && ( ( $direction == false && $page == 1 ) || ( $direction == true && $page == $numberOfPages ) ) ) {
...
//		if(! is_rtl())
    ...
    public static function get_navigation_dropdown()
    {
        global $post, $page, $pages;

        if (!$post || count($pages) <= 1) {
            return '';
        }

        $dropdownOptions = [];
        for ($i = 1; $i <= count($pages); $i++) {
            $subPage = Misc::get_sub_page($i, $page);
            $selected = $i === $page ? 'selected' : '';
            $dropdownOptions[] =
                '<option value="' . $i . '" ' . $selected . '>' .
                esc_html(strip_tags($subPage['shortCodeTitle'])) .
                '</option>';
        }
        ob_start();
        ?>
        <div class="theiaPostSlider_dropdown">
            <select><?= implode('', $dropdownOptions) ?></select>
        </div>
        <?php
