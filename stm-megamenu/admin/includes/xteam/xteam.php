...
class STM_Nav_Menu_Item_Custom_Fields
{
    ...
    static function view_control($field) {
        ...
        switch ($type) {
			...
			case 'checkbox':
				...
				break;
            case 'separator':
                $view .= '<h4 class="stm-separator stm_visible_lvl_0">
                    {label}
                </h4>';
                break;
            case 'tips':
    ...
    static function get_field($item, $depth, $args)
    {
        ...
        foreach (self::$options['fields'] as $field) {
        	...
        	if($field['name'] == 'stm_menu_enable_cat_mm' && get_post_meta($item->ID, self::get_menu_item_postmeta_key($field['name']), true) == "checked") $field['checked'] = "checked";
        	$field['id'] = $item->ID;
