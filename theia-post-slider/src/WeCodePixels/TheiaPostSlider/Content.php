...
class Content {
    public static function staticConstruct() {
        ...
        add_action( 'the_content', __NAMESPACE__ . '\\Content::the_content_start', -100000000 );
        add_action( 'the_content', __NAMESPACE__ . '\\Content::the_content', 999999 );
        add_action( 'the_content', __NAMESPACE__ . '\\Content::the_content_end', 100000000 );
    ...
    // This is used to prevent the_content() from calling itself in an infinite loop, and to defend against other plugins that make nested calls to the_content.
    public static $theContentNesting = 0;
    ...
    public static function the_content_start( $content ) {
        self::$theContentNesting++;
        return $content;
    }

    public static function the_content_end( $content ) {
        self::$theContentNesting--;
        return $content;
    }

    public static function the_content_early( $content ) {
        if (self::$theContentNesting > 1) return $content;
    ...
    public static function the_content( $content ) {
        ...
        // Add slider HTML.
        {
            ...
            if (in_array(PostOptions::get($post->ID, 'dropdown_vertical_position', 'tps_nav'), ['top_and_bottom', 'top'])) {
                $html .= NavigationBar::get_navigation_dropdown();
            }
            ...
            if (in_array(PostOptions::get($post->ID, 'dropdown_vertical_position', 'tps_nav'), ['top_and_bottom', 'bottom'])) {
                $html .= NavigationBar::get_navigation_dropdown();
            }
        ...
        if ( PostOptions::get( $post->ID, 'slide_loading_mechanism', 'tps_advanced' ) != 'refresh' ) {
            $sliderOptions = array(
                ...
                'keyboardShortcuts'          => Misc::is_compatible_post() && ! Options::get( 'disable_keyboard_shortcuts', 'tps_nav' ),
            ...
            if ( Options::get( 'window_load_js' ) ) {
                $onChangeSlide .= "jQuery(window).load();";
            }
            if ( Options::get( 'document_ready_js' ) ) {
                $onChangeSlide .= "jQuery(document).ready();";
            }
            ...
            if ( Options::get( 'document_resize_js' ) ) {
                $onChangeSlide .= "jQuery(document).resize();";
            }
            if ( Options::get( 'document_scroll_js' ) ) {
                $onChangeSlide .= "jQuery(document).scroll();";
            }
            if ( $custom = Options::get( 'document_custom_js' ) ) {
                $onChangeSlide .= $custom;
            }

            $html .= "<div data-theia-post-slider-options='" . htmlspecialchars( json_encode( $sliderOptions ), ENT_QUOTES ) . "'
					 data-theia-post-slider-on-change-slide='" . htmlspecialchars( json_encode( $onChangeSlide ), ENT_QUOTES ) . "'></div>";
