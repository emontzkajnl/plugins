...
class Templates {
    ...
    public static function getDropdownNavigationHtml( $currentOptions, $postPage = false ) {
        $prefix   = $postPage ? 'tps_options' : 'tps_nav';
        $onchange = $postPage ? '' : 'updateSlider()';
        ?>
        <tr valign="top">
            <th scope="row">
                <label for="tps_nav_vertical_position"><?php _e( "Dropdown navigation:", 'theia-post-slider' ); ?></label>
            </th>
            <td>
                <select id="tps_nav_vertical_position"
                        name="<?php echo $prefix; ?>[dropdown_vertical_position]"
                        onchange="<?php echo $onchange; ?>">
                    <?php
                    $options = array();
                    if ( $postPage ) {
                    ...
                    $options = array_merge( $options, Options::get_button_vertical_positions() );
                    foreach ( $options as $key => $value ) {
                        $output = '<option value="' . $key . '"' . ( $key == $currentOptions['dropdown_vertical_position'] ? ' selected' : '' ) . '>' . $value . '</option>' . "\n";
                        echo $output;
                    }
                    ?>
                </select>
                <p class="description">
                    <?=__('The dropdown is useful both as a summary and to quickly navigate between slides.', 'theia-post-slider')?>
                </p>
            </td>
        </tr>
        <?php
