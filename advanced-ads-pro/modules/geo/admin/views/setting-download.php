...
if ( ! $correct_databases ) :
	?><p class="advads-notice-inline advads-error"><?php _e( 'Geo Databases not found.', 'advanced-ads-pro' ); ?></p>
	<p><?php _e( 'In order to use Geo Targeting, please download the geo location databases by clicking on the button below.', 'advanced-ads-pro' ); ?></p>
				 <?php
else :
	if ( $last_update ) :
		?>
		<p class="advads-notice-inline advads-check"><?php printf( __( 'Last update: %s', 'advanced-ads-pro' ), date_i18n( get_option( 'date_format' ), $last_update ) ); ?></p>
		<?php
	endif;
endif;
...
if ( $this->is_update_available() || ! $correct_databases ) :
	?>
<button type="button" id="download_geolite" class="button-secondary"><?php _e( 'Update geo location databases', 'advanced-ads-pro' ); ?> (~66MB)</button>
...
<script>
	...
	?>
	<p><?php printf( __( 'Next possible update on %s.', 'advanced-ads-pro' ), date_i18n( get_option( 'date_format' ), $next_update ) ); ?></p>
	<p class="description"><?php _e( 'The databases are updated on the first Tuesday (midnight, GMT) of each month.', 'advanced-ads-pro' ); ?></p>
	<?php
endif;
