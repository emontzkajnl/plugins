registerBlockType('stm-gutenberg/widget-author', {
    title: 'STM Widget Author',
    ...
    edit (props) {
        ...
        var authorImg = attributes.author_img;
        var authorSubtitle = attributes.author_subtitle;
        var authorName = attributes.author_name;
        var authorDesc = attributes.author_desc;
        ...
        return[
            <ServerSideRender block="stm-gutenberg/widget-author" attributes={props.attributes}/>,
            <InspectorControls key="inspector">
                ...
                <PanelBody title={ i18n.__( 'View Settings' ) } className='stm-posts-view-settings' initialOpen={ false }>
                    <SelectControl label={ i18n.__( 'View Style' ) } value={ viewStyle } options={ authorStyles } onChange={ (selectedStyle) => props.setAttributes( { viewStyle: selectedStyle } ) }/>
                    <div>CSS Box</div>
                ...
                <PanelBody title={ i18n.__( 'Author Settings' ) } className='stm-categories-settings' initialOpen={ false }>
                    <TextControl type='text' label={ i18n.__('Author img Link') } value={ authorImg } onChange={ (newImg) => props.setAttributes( { author_img: newImg } ) }/>
                    <TextControl type='text' label={ i18n.__('Author SubTitle') } value={ authorSubtitle } onChange={ (newSubtitle) => props.setAttributes( { author_subtitle: newSubtitle } ) }/>
                    <TextControl type='text' label={ i18n.__('Author Title') } value={ authorName } onChange={ (newName) => props.setAttributes( { author_name: newName } ) }/>
                    <TextareaControl type='text' label={ i18n.__('Author Description') } value={ authorDesc } onChange={ (newDesc) => props.setAttributes( { author_desc: newDesc } ) }/>
                </PanelBody>
            </InspectorControls>
        ];
    },
