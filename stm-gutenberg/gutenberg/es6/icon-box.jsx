class StmIconPicker extends React.Component {
    ...
    render () {
        return (
            <div className="stm-icon-picker">
                <input id="icon_set" onChange={ this.onChangeInput.bind(this) } type="text" value={this.state.icon} />
                <select className="c-select-field__menu" value={ this.state.icon } onChange={ this.onChangeIco.bind(this) } id="icon_field">
                    { this.icons.map((ico, index) => {
                        return (
                            <option key={ index } value={ ico.value }>{ ico.name }</option>
                        );
                    }) }
                </select>
            </div>
...
registerBlockType('stm-gutenberg/icon-box', {
    ...
    edit: function (props) {
        ...
        return [
            <ServerSideRender block='stm-gutenberg/icon-box' attributes={props.attributes} />,
            <InspectorControls key='inspector'>
                ...
                <PanelBody title={ i18n.__( 'icon Box Settings' ) } className="stm-list-view" initialOpen={ true }>
                    <StmIconPicker />
                </PanelBody>
            </InspectorControls>
        ];
